...
- [T-249](troubleshooting/T-249.md) · `document.fonts.check()`는 폰트가 안 떠도 true를 준다 — 「폰트 로드됨」을 확인했다고 믿었는데 실제로 그려지는 글꼴은 알 수 없었다
- [T-248](troubleshooting/T-248.md) · 돌연변이 테스트에서 PowerShell 인용부 오류로 치환이 조용히 안 먹었는데 결과가 「67 통과」로 나왔다 — 그대로 읽으면 「테스트가 이 결함을 못 잡는다」는 정반대 결론이 된다
