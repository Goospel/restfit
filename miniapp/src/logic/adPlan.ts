...
/**
 * 광고를 띄우기 전에 예고 문구를 보여 주는 시간(ms).
 *
 * 4차 검수 반려 사유가 **「예상하기 어려운 시점의 광고」**였다(T-245). 광고는 그대로 자동으로
 * 뜨되, 뜨기 전에 화면이 먼저 말할 시간을 준다.
 *
 * ⚠️ **타이머는 안 건드린다.** 휴식은 절대 시각(`restEndsAt`)으로 그대로 흐르므로, 이 3초는
 * 사용자에게서 시간을 빼앗지 않는다 — 늦춰지는 것은 노출뿐이다.
 */
export const AD_NOTICE_MS = 3000;
