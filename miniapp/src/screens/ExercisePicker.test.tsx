...
describe('내 운동 고르기', () => {
  ...
  /**
   * 규칙은 화면 상단 안내가 말한다. 이 줄은 **지금 고른 이 조합이 실제로 나뉘는지**를 말한다 —
   * 「읽었다」와 「내 것이 됐다」의 차이라, 고른 직후 여기서 확인이 안 되면 홈에서 반쪽만
   * 뜨는 것을 보고서야 알게 된다.
   */
  it('밀기와 당기기를 함께 고르면 「하루씩 번갈아」라고 알려 준다', () => {
    setup(['Pullups', 'Pushups']);
    expect(screen.getByText('고른 운동 2개 · 하루씩 번갈아')).toBeTruthy();
  });

  it('한쪽만 고르면 번갈아 간다고 하지 않는다 — 실제로 안 나뉜다', () => {
    setup(['Pushups', 'Bench_Dips']);
    expect(screen.getByText('고른 운동 2개')).toBeTruthy();
  });
