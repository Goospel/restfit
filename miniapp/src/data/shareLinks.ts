...
/**
 * 상품을 누르기 **전에** 돌아오는 길을 알려 준다.
 *
 * ⚠️ **뒤로 가기로는 이 앱에 못 돌아온다.** 링크가 `Device.openURL`로 열리면 토스쇼핑이 자기 스택으로
 * 뜨는데, 미니앱은 그 스택에 없어서 뒤로 가기가 토스쇼핑 안에서만 움직인다. 고칠 방법이 우리에게 없다 —
 * `Device.openURL`은 URL 문자열 하나만 받고(복귀 경로를 줄 인자가 없다), 외부 주소를 미니앱 안에서 여는
 * SDK도 없다(웹뷰 API는 `openPDFViewer` 하나뿐이고 PDF 전용). 토스 공식 답변도 *"토스 내 다른 서비스로
 * 이동하는 별도의 SDK / API는 제공해드리고 있지 않습니다"* 다.
 *
 * 토스가 **내부 개선 중**이라고 답한 알려진 문제이므로([T-228](../../../claude-docs/troubleshooting/T-228.md)),
 * 고쳐지면 이 상수와 쓰는 자리를 지운다. 그때까지는 **길을 잃는 것보다 한 줄 읽는 편이 낫다.**
 */
export const RETURN_NOTICE = '상품을 열면 뒤로 가기로는 못 돌아와요. 토스 홈에서 다시 열어주세요.';
